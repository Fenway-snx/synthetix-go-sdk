...
func (c *Client) resolveSubAccount(ctx context.Context, o options) (uint64, error) {
	...
	if len(ids) > 1 {
		c.logger.Warn("synthetix: wallet has multiple subaccounts; using first returned by API — set Config.SubAccountID explicitly in production",
			"count", len(ids), "selected", ids[0])
	}
